...
export class ParametersJarFactory {
  ...
  public createFromOperation (operation: Operation): ParametersJar {
    return {
      ...
      bodyParams: this.getOperationParametersByType(operation, PARAMETER_TYPE_BODY),
      formDataParams: this.getOperationParametersByType(operation, PARAMETER_TYPE_FORM_DATA),
      headerParams: this.getOperationParametersByType(operation, PARAMETER_TYPE_HEADER),
    }
  }
    ...
    return (operation.parameters || [])
      .map((parameter: Parameter & { '$ref': string }) => {
        if (parameter.$ref) {
          ...
          const referred = this.swagger.parameters[segments.length === 1 ? segments[0] : segments[2]]
          if (!referred) {
