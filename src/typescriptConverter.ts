...
export class TypescriptConverter implements BaseConverter {
  ...
  public generateOperation (path: string, method: string, operation: Operation) {
    ...
    const parameters: string[] = pathParams.map((parameter) => {
      return `${parameter.name}${PARAMETER_PATH_SUFFIX}: ${this.generateTypeValue(parameter as any as Schema)}`
    })
    const args: string[] = [PARAMETER_TYPE_PATH]

    if (this.settings.allowVoidParameters || queryParams.length > 0) {
      parameters.push(`${PARAMETER_TYPE_QUERY}: ${name}${PARAMETERS_QUERY_SUFFIX}`)
    ...
    if (this.settings.allowVoidParameters || bodyParams.length > 0) {
      parameters.push(`${PARAMETER_TYPE_BODY}: ${name}${PARAMETERS_BODY_SUFFIX}`)
    ...
    if (this.settings.allowVoidParameters || formDataParams.length > 0) {
      parameters.push(`${PARAMETER_TYPE_FORM_DATA}: ${name}${PARAMETERS_FORM_DATA_SUFFIX}`)
    ...
    if (this.settings.allowVoidParameters || headerParams.length > 0) {
      parameters.push(`${PARAMETER_TYPE_HEADER}: ${name}${PARAMETERS_HEADER_SUFFIX}`)
  ...
  public generateTypeValue (definition: Schema & { schema?: Schema }): string {
    ...
    switch (definition.type) {
      ...
      case DEFINITION_TYPE_OBJECT: {
        ...
        const hasProperties = definition.properties && Object.keys(definition.properties).length > 0
        ...
        if (hasProperties) {
          ...
          output += Object
            ...
            .map(([name, def]) => {
              ...
              return `'${name}'${isRequired ? '?' : ''}: ${this.generateTypeValue(def)}`
            })
            .join('\n')
          output += '\n}'
        }
...
`
    ...
    output += Object.entries(this.swagger.paths).map(([path, methods]) => {
